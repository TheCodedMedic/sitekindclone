import { motion } from 'framer-motion';

export function Scene5() {
  return (
    <motion.div 
      className="absolute inset-0 flex items-center justify-between px-[8vw] z-50 bg-[var(--color-mint)] overflow-hidden"
      initial={{ clipPath: "polygon(0 100%, 100% 100%, 100% 100%, 0 100%)" }}
      animate={{ clipPath: "polygon(0 0, 100% 0, 100% 100%, 0 100%)" }}
      exit={{ x: "-100%", transition: { duration: 0.8, ease: [0.76, 0, 0.24, 1] } }}
      transition={{ duration: 1, ease: [0.76, 0, 0.24, 1] }}
    >
      <div className="mesh opacity-40" />
      <div className="dot-grid opacity-30" />
      
      <motion.div 
        className="w-[45%] relative z-10 flex justify-center items-center"
        initial={{ opacity: 0, y: "5vw" }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.5, duration: 1.2, ease: [0.34, 1.3, 0.64, 1] }}
      >
        <img 
          src={`${import.meta.env.BASE_URL}images/maps-pin.png`}
          className="w-[30vw] object-contain drop-shadow-2xl"
          alt="Map Pin"
        />
      </motion.div>

      <div className="w-[45%] relative z-10 flex flex-col justify-center">
        <motion.div
          className="font-accent text-[3vw] text-[var(--color-accent)] mb-[1vw]"
          initial={{ opacity: 0, x: "2vw" }}
          animate={{ opacity: 1, x: 0 }}
          transition={{ delay: 0.8 }}
        >
          Dominate local search.
        </motion.div>
        <motion.h2 
          className="font-display text-[4.5vw] font-extrabold text-ink leading-tight"
          initial={{ opacity: 0, y: "3vw" }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 1, duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
        >
          Automated SEO & Maps.
        </motion.h2>
        <motion.p
          className="font-body text-[2.2vw] text-ink-2 mt-[1.5vw]"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ delay: 1.6 }}
        >
          Rank higher. Get found. <br/>
          <strong className="text-[var(--color-primary)]">Zero effort required.</strong>
        </motion.p>
      </div>

    </motion.div>
  );
}
